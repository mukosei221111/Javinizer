function Get-DmmUrl {
    ...
    process {
        ...
        # } else {
            ...
            $searchResults = $webRequest.links.href | Where-Object { $_ -match '\/mono\/dvd' -or $_ -match '\/digital\/video' }

            if ($searchResults) {
                $matchedResults = @()

                # Prioritize digital video before falling back to DVD matches
                if ($searchResults -match "\/digital\/videoc") {
                    $searchResults | Where-Object { $_ -match "\/digital\/videoc" -and $_ -match $contentId } | ForEach-Object {
